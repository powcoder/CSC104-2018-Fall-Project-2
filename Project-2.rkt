...
; ★ Define textify:
(define (textify x)
  (if ((number? x) (number->text x)) (else x)))
...
; ★ Define imagine:
(define (imagine x)
  (local ((define board (invisible-width text-size))
          (define textImg (text->image (textify x) text-size text-color)))
    (beside board textImg board)))
...
(check-expect (column (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)) 2)
              (list 1.2 2.2 3.3))
...
(check-expect (column (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)) 2)
              (local ((define (myElement row)
                        (element row 2)))
                (map myElement (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)))))
...
(check-expect (columns (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)))
              (list (list "red" "blue" "xxx")  (list "rec" "gg" "yyy") (list 1.2 2.2 3.3)))

; ★ Write a Design check-expect for the function. Hint: see the hint for ‘column’.

(check-expect (columns (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)))
              (local ((define (getSeq n)
                        (if ((< n 0) (list))
                            (else (join (getSeq (- n 1)) (list n)))))
                      (define seq ( getSeq (- (length (first (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)))) 1)))
                      (define (myColumn i)
                        (column (list (list "red" "rec" 1.2) (list "blue" "gg" 2.2) (list "xxx" "yyy" 3.3)) i)))
                (map myColumn seq)))
...
(define (columns table)
  (local ((define (getSeq n)
            (if ((< n 0) (list))
                (else (join (getSeq (- n 1)) (list n)))))
          (define seq ( getSeq (- (length (first table)) 1)))
          (define (myColumn i)
            (column table i)))
    (map myColumn seq)))
...
(check-expect (number? (text->number "10.4")) #true)
(check-expect (number? (text->number "ten point four")) #false)
...
(define (convert-numeric a-text)
  (local ((define t (text->number a-text)))
    (if ((number? t) t)
        (else a-text))))
...
(check-expect (if-numeric-list (list) "numbers" "something else")  "something else")

(check-expect (if-numeric-list (list "good" 123) "numbers" "something else")  "something else")

; ★ Define ‘if-numeric-list’:
(define (if-numeric-list entries choice-for-numeric other-choice)
  (if ( (and (not (empty? entries)) (number? (first entries)))  choice-for-numeric)
      (else other-choice)))
...
(define (alignment a-list)
  (if-numeric-list a-list "right" "left"))
...
; ★ Define ‘frame’:
(define (frame image)
  (overlay (rectangle (* frame-proportion (width image)) (* frame-proportion (height image)) "outline" "black")
           image))
...
(check-expect (text-match? "it" "kitten") (sub-text? (lower-case "it") (lower-case "kitten")))
(check-expect (text-match? "It" "kitten") (sub-text? (lower-case "It") (lower-case "kitten")))
(check-expect (text-match? "bit" "kitten") (sub-text? (lower-case "bit") (lower-case "kitten")))
(check-expect (text-match? "Troll" "UTRoll") (sub-text? (lower-case "Troll") (lower-case "UTRoll")))

; ★ Define ‘text-match?’:
(define (text-match? part text)
  (sub-text? (lower-case part) (lower-case text)))
...
(define (ordering entries)
  (if-numeric-list entries < alphabet-order?))
...
(define (table-sort table index)
  (local [(define ordered? (ordering (column table index)))
          (define (orderRow? r1 r2)
            (ordered? (element r1 index) (element r2 index)))]
    (sort orderRow? table)))
...
; ★ Define ‘bar’:
(define (bar amount)
  (overlay (rectangle (* (/ amount 2) text-size) text-size "solid" "transparent")
           (rectangle (* (/ amount 2) text-size) (/ text-size 2) "solid" bar-color)))
...
(check-expect (unique (list))
              (if ((empty? (list)) (list))
                  (else (adjoin (first (list)) (unique (remove (first (list)) (rest (list))))))))
...
(check-expect (unique (list "b" "a" "c" "b" "b" "d"))
              (if ((empty? (list "b" "a" "c" "b" "b" "d")) (list))
                  (else (adjoin (first (list "b" "a" "c" "b" "b" "d")) (unique (remove (first (list "b" "a" "c" "b" "b" "d")) (rest (list "b" "a" "c" "b" "b" "d"))))))))

(check-expect (unique (list "a" "c" "b" "b" "d"))
              (if ((empty? (list "a" "c" "b" "b" "d")) (list))
                  (else (adjoin (first (list "a" "c" "b" "b" "d")) (unique (remove (first (list "a" "c" "b" "b" "d")) (rest (list "a" "c" "b" "b" "d"))))))))
...
(define (unique a-list)
  (if ((empty? a-list) (list))
      (else (adjoin (first a-list) (unique (remove (first a-list) (rest a-list)))))))
...
(define (counts entries)
  (local ((define len (length entries))
          (define uniqEntries (unique entries))
          (define (numElement e)
             (- len (length (remove e entries))))
          (define (getPair e)
              (list e (numElement e))))
    (map getPair uniqEntries)))
